...
module.exports = class NightwatchApi {
  ...
  _startSession (options) {
    ...
    this.hookAssertion(this.options.nightwatchOutput)
  ...
  * takeScreenshot (moduleName, testName) {
  ...
  hookAssertion (shouldLog) {
    ...
    this.client.get().assertion = function (passed, receivedValue, expectedValue, message, abortOnFailure, originalStackTrace) {
      ...
      const val = originalAssertion.apply(this, arguments)
      if (!passed && !abortOnFailure) {
        const tests = self.client.get().results.tests;
        tests[tests.length - 1].ignored = false
      }
      return val
    }
  }
  ...
  start () {
    return new Promise((resolve, reject) => {
      this.client.get().once('nightwatch:finished', (results, errors) => {
        const errorList = results.tests.filter(test => test.failure && !test.ignored).map(test => {
